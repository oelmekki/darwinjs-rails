...
But it does not make sense to think in term of model anyway. Data we handle on
the user end has nothing to do with raw data from our database, it's more like
formatted data generated by our helpers and presenters when it comes, and like
params content when it goes. We don't have to compute data, all is already done
on server side. Your server side app is built around a database. Your client
side app is built around the html description language. It's a presenter that
